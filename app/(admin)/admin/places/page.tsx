...
export default async function PlacesPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return (
    <div>
      <PageHeader title={`Places (${totalCount})`}><SearchBar placeholder="Search places by name or city..." /></PageHeader>
      <Card>
        ...
        <ActionForm action={createPlace} submitLabel="Create Place">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            ...
            <FormGroup label="Featured?"><Select name="is_featured"><option value="false">No</option><option value="true">Yes</option></Select></FormGroup>
          </div>
          <FormGroup label="Full Description"><TextArea name="description" rows={3} placeholder="Detailed description..." /></FormGroup>
        </ActionForm>
      </Card>
        ...
        <DataTable headers={['Name', 'Region', 'Type', 'Rating', 'Active', 'Featured', 'Actions']}>
          {places.map((p: any) => (
            <EditableRow key={p.id} id={p.id} editAction={updatePlace} deleteAction={deletePlace}
              ...
              fields={[
                { name: 'region_id', value: p.region_id, options: regionOptions }, { name: 'name', value: p.name }, { name: 'slug', value: p.slug },
