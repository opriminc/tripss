...
import { createPlace, deletePlace } from '@/app/actions/admin'
import { PageHeader, DataTable, Td, FormGroup, Input, Select, TextArea, Card, ActionForm, DeleteButton } from '../_components/admin-ui'

export default async function PlacesPage() {
  const supabase = createAdminClient()
  const [{ data: places }, { data: regions }, { data: travelTypes }] = await Promise.all([
    supabase.from('places').select('*, regions(name), travel_types(name)').order('name'),
    supabase.from('regions').select('id, name').order('name'),
    supabase.from('travel_types').select('id, name').order('display_order'),
  ])
      ...
      <Card>
        ...
        <ActionForm action={createPlace} submitLabel="Create Place">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            ...
            <FormGroup label="Short Description"><Input name="short_description" placeholder="Brief description for cards" /></FormGroup>
        ...
        <DataTable headers={['Name', 'Region', 'Type', 'Rating', 'Active', 'Featured', 'Actions']}>
          {places?.map(p => (
            <tr key={p.id}>
              ...
              <Td><DeleteButton action={deletePlace} id={p.id} confirmMessage={`Delete "${p.name}"? All images, ratings, and interest links will also be deleted.`} /></Td>
            </tr>
          ))}
        </DataTable>
      </div>
