...
const CONTACT_TYPES = [
  { value: 'website', label: 'Website' }, { value: 'phone', label: 'Phone' }, { value: 'email', label: 'Email' },
  { value: 'facebook', label: 'Facebook' }, { value: 'instagram', label: 'Instagram' }, { value: 'twitter', label: 'Twitter' },
  { value: 'booking', label: 'Booking' }, { value: 'other', label: 'Other' },
]

export default async function PlaceContactsPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  const { q, page } = await searchParams
  const [{ data: contacts, totalCount }, places] = await Promise.all([
    queryPaginated({ table: 'place_contacts', select: '*, places(name)', page: parseInt(page ?? '1'), search: q, searchColumns: ['value', 'label'], orderBy: 'created_at', ascending: false }),
  ...
  return (
    <div>
      <PageHeader title={`Place Contacts (${totalCount})`}><SearchBar placeholder="Search by URL, phone, label..." /></PageHeader>
      <Card>
        ...
        <ActionForm action={createPlaceContact} submitLabel="Add Contact">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            ...
            <FormGroup label="Type"><Select name="type" required>{CONTACT_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</Select></FormGroup>
      ...
      <div style={{ marginTop: '24px' }}>
        <DataTable headers={['Place', 'Type', 'Value', 'Label', 'Primary', 'Active', 'Actions']}>
          {contacts.map((c: any) => (
            <EditableRow key={c.id} id={c.id} editAction={updatePlaceContact} deleteAction={deletePlaceContact}
              fields={[
                { name: 'place_id', value: c.place_id, options: placeOptions }, { name: 'type', value: c.type, options: CONTACT_TYPES },
                { name: 'value', value: c.value }, { name: 'label', value: c.label ?? '' },
              ...
              renderView={<><Td>{c.places?.name}</Td><Td>{c.type}</Td><Td>{c.value?.length > 40 ? c.value.slice(0, 40) + '...' : c.value}</Td><Td>{c.label ?? '—'}</Td><Td>{c.is_primary ? 'Yes' : 'No'}</Td><Td>{c.is_active ? 'Yes' : 'No'}</Td></>}
            />
          ))}
          {contacts.length === 0 && (<tr><Td>No contacts found</Td><Td /><Td /><Td /><Td /><Td /><Td /></tr>)}
        </DataTable>
