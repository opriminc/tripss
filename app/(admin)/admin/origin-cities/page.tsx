...
export default async function OriginCitiesPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  const { q, page } = await searchParams
  const [{ data: cities, totalCount }, regions] = await Promise.all([
    queryPaginated({ table: 'origin_cities', select: '*, regions(name)', page: parseInt(page ?? '1'), search: q, searchColumns: ['name', 'slug'] }),
  ...
  return (
    <div>
      <PageHeader title="Origin Cities"><SearchBar placeholder="Search cities..." /></PageHeader>
      <Card>
        ...
        <ActionForm action={createOriginCity} submitLabel="Create City">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            <FormGroup label="Region"><Select name="region_id" required><option value="">Select region</option>{(regions as any[]).map((r: any) => <option key={r.id} value={r.id}>{r.name}</option>)}</Select></FormGroup>
            ...
            <FormGroup label="Default?"><Select name="is_default"><option value="false">No</option><option value="true">Yes</option></Select></FormGroup>
          </div>
        </ActionForm>
      </Card>
      <div style={{ marginTop: '24px' }}>
        <DataTable headers={['Name', 'Slug', 'Region', 'Lat/Lng', 'Default', 'Active', 'Actions']}>
          {cities.map((c: any) => (
            <EditableRow key={c.id} id={c.id} editAction={updateOriginCity} deleteAction={deleteOriginCity}
              fields={[
                { name: 'region_id', value: c.region_id, options: regionOptions }, { name: 'name', value: c.name }, { name: 'slug', value: c.slug },
                { name: 'lat', value: c.lat, type: 'number' }, { name: 'lng', value: c.lng, type: 'number' },
              ...
              renderView={<><Td>{c.name}</Td><Td>{c.slug}</Td><Td>{c.regions?.name}</Td><Td>{c.lat}, {c.lng}</Td><Td>{c.is_default ? 'Yes' : 'No'}</Td><Td>{c.is_active ? 'Yes' : 'No'}</Td></>}
            />
          ))}
        </DataTable>
